...
namespace _7._04.TopServera
{
    ...
    class PlayerDatabase
    {
        ...
        public void Work()
        {
            ...
            const string CommandShowTopLevel = "1";
            const string CommandShowTopPower = "2";
            ...
            while (isExit == false)
            {
                ...
                Console.WriteLine($"\nДоступные команды:" +
                    ...
                    $"\n{CommandShowTopLevel}. Показать Топ - 3 по уровню" +
                    $"\n{CommandShowTopPower}. Показать Топ - 3 по силе");
                Console.WriteLine();
                ...
                switch (Console.ReadLine())
                {
                    ...
                    case CommandShowTopLevel:
                        ShowTopPlayers(_players.OrderByDescending(player => player.Level).ToList());
                        break;

                    case CommandShowTopPower:
                        ShowTopPlayers(_players.OrderByDescending(player => player.Power).ToList());
                        break;
        ...
        private void ShowTopPlayers(List<Player> players)
        {
            ...
            Console.Clear();
            ShowPlayers(players.Take(topPlayersMax).ToList());
            Console.ReadKey();
        }

        private void ShowPlayers(List<Player> patients)
        {
            foreach (Player patient in patients)
            {
                patient.ShowInfo();
                Console.WriteLine();
            }
