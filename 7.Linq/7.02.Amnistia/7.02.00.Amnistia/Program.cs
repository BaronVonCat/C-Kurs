...
namespace _7._02.Amnistia
{
    ...
    class PrisonersDatabase
    {
        ...
        private void TryAmnesty()
        {
            ...
            if (TryGetPrisonersByArticle(out prisoners))
            {
        ...
        private bool TryGetPrisonersByArticle(out List<Prisoner> prisoners)
        {
            bool isPrisonersFound = false;
            int userNumber = 0;
            ...
            {
                List<Prisoner> filteredPrisoners = _prisoners.Where(prisoner => prisoner.Offense == _offenses[userNumber - 1]).ToList();

                if (filteredPrisoners.Count > 0)
                {
                    prisoners = filteredPrisoners;
                    isPrisonersFound = true;
                }
            }
            else
            {
        ...
        private void AmnestyPrisoners(List<Prisoner> prisoners)
        {
            _prisoners.RemoveAll(prisoner => prisoners.Contains(prisoner));
            Console.Clear();
